...
class TrickleStreamDecoder:
    ...
    def process_segment(self, segment_data: bytes) -> List[av.VideoFrame]:
        """Process trickle segment into video frames"""
        try:
            if not segment_data:
                return []
            ...
            for packet in container.demux(video_stream):
                for frame in packet.decode():
                    ...
                    if (frame.width != self.target_width or 
            ...
            logger.debug(f"Decoded {len(frames)} frames from segment")
            return frames
...
class TrickleSegmentEncoder:
    ...
    def encode_frames(self, frames: List[av.VideoFrame]) -> bytes:
        """Encode frames into trickle segment"""
        try:
            if not frames:
                return b""
            ...
            # Set codec options for streaming
